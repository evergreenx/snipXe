...
import {  handleFontSizeUpdate, handleVPaddingUpdate } from "@/sm/features/control/controlSlice";
...
const FontSizeControl = () => {
  ...
  const handleSelectedFontSize = (value: string) => {
    dispatch(handleFontSizeUpdate(value));
    setValue(value);
  };



  const [value, setValue] = React.useState(selectedFontSize);
  return (
    <Select.Root
      onValueChange={handleSelectedFontSize}
      defaultValue={selectedFontSize}
      value={value}
    >
      <Select.Trigger
        className="inline-flex w-[77px] pl-4 text-center  items-center justify-between rounded mt-[20px]  text-base font-semibold leading-none   bg-transparent text-secondary border-[0.5px] border-[#DDE1E1] outline-none"
        aria-label="vpadding"
      >
        <Select.Value
          aria-label={value}
